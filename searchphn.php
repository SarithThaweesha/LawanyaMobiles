...
<body>
	...
	if(isset($_POST['sebut'])){
		$search=mysqli_real_escape_string($conn,$_POST['seinput']);
		$sql="SELECT * FROM product WHERE Brand LIKE '%$search%' OR ProductName LIKE '%$search%' OR Category LIKE '%$search%'";
		$result = mysqli_query($conn, $sql);
		$queryResult=mysqli_num_rows($result);
		
		if($queryResult > 0){
			while($row=mysqli_fetch_assoc($result)){
		component($row['ProductName'],$row['Price'],$row['image_url'],$row['ImgIDd'],$row['Brand'],$row['Quantity']); 
			}
		}else{
